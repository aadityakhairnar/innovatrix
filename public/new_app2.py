    ...
    try:
        ...
        if loan_application_file:
            ...
            questions = {
                ...
                "profile_score": "Fetch the profile score"
            }
            extracted_data = extract_data_from_pdf(BytesIO(pdf_bytes), questions)
...
# Construct JSON result
        credit_memo_prompt = f"""
            ...
            Loan Amount: {loan_amount}
            Loan Purpose: {loan_purpose}
            Profile Score: {extracted_data.get('profile_score', 'N/A')}
        ...
        try:
            # Bank Statement and Credit Score Analysis for Business Loan
            if bank_statement_file:
                bank_statement_pdf = bank_statement_file.read()
                bank_statement_insights = analyze_transactions(extract_transactions_from_pdf(BytesIO(bank_statement_pdf)))
            ...
            if credit_score_certificate_file:
                credit_score_pdf = credit_score_certificate_file.read()
                credit_score_text = get_pdf_text(BytesIO(credit_score_pdf))
                credit_score_analysis = f"Credit Score Analysis:\n{credit_score_text}"
